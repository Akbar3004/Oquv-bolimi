...
import { LayoutDashboard, BookOpen, GraduationCap, FileBarChart, Users, ChevronLeft, ChevronRight } from 'lucide-react';
import { NavLink } from 'react-router-dom';

const navItems = [
    { name: 'Asosiy', path: '/', icon: <LayoutDashboard size={20} /> },
    { name: 'Dars', path: '/lessons', icon: <BookOpen size={20} /> },
    { name: 'Imtihon', path: '/exams', icon: <GraduationCap size={20} /> },
    { name: 'Hisobotlar', path: '/reports', icon: <FileBarChart size={20} /> },
    { name: 'Xodimlar', path: '/workers', icon: <Users size={20} /> },
];
    ...
    return (
        <div className={`h-[calc(100vh-64px)] fixed bottom-0 left-0 bg-[hsl(var(--card))] border-r border-[hsl(var(--border))] transition-all duration-300 md:relative md:h-full z-40 ${isOpen ? 'w-64' : 'w-20'}`}>
            <div className="flex flex-col h-full py-6 px-3">
                {/* Toggle Button Inside Sidebar (optional) */}
